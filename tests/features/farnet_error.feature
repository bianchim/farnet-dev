Feature: Farnet custom error
  ...
  Scenario: Check Public Access denied page
    ...
    Given I am logged in as a user with the "administrator" role
    And I go to "admin/config/system/farnet_error"
    And I fill in "farnet_error_myfarnet_title" with "Access denied MyFarnet"
    And I fill in "farnet_error_myfarnet_body[value]" with "You don't have access to MyFarnet content"
    When I press the "Save configuration" button
    Then I should see the success message "The configuration options have been saved."
    When I click "French"
    And I fill in "farnet_error_myfarnet_title" with "Accès Refusé MyFarnet"
    And I fill in "farnet_error_myfarnet_body[value]" with "Vous n'avez pas accès au contenu MyFarnet"
    When I press the "Save configuration" button
    Then I should see the success message "The configuration options have been saved."
    Given I am an anonymous user
