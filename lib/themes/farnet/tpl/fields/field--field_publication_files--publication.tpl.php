  ...
  <ul class="publication-files-list list-inline">
    <?php foreach ($items as $delta => $item_collection): ?>
      <?php foreach ($item_collection['entity']['field_collection_item'] as $delta => $item): ?>
        ...
        <?php foreach ($languages as $country => $country_class) : ?>
          <?php if ($country === $item['field_term_languages_spoken'][0]['#title']) : ?>
            <?php $country_code = $country_class->language; ?>
          <?php endif; ?>
        <?php endforeach; ?>
          <?php if (!empty($item['field_publication_file'][0]['#markup'])) : ?>
          <li>
            <span class="file">
