  ...
  Drupal.behaviors.farnet_gp_project = {
    attach: function () {
      ...
      var calculateB = function () {
        ...
        var b1 = 0;
        $(idB1 + " .field-type-number-decimal input").each(function () {
          var b1i = ($(this).val().length > 0) ? parseFloat($(this).val().replace(',', '.')) : 0;
          b1 = b1 + b1i;
        });

        totalB = b2 + b1;
        $(idTotalB).val(totalB.toFixed(2).replace('.', ','));
